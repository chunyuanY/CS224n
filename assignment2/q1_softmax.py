...
def softmax(x):
    ...
    ex = tf.exp(x - x_max)
    out = ex/tf.reduce_sum(ex, axis=1, keep_dims=True)
    ### END YOUR CODE
...
def cross_entropy_loss(y, yhat):
    ...
    ### YOUR CODE HERE
    y = tf.to_float(y) # tensorflow bug!, 如果不转换会出错： ValueError: Tensor conversion requested dtype int32 for Tensor with dtype float32
